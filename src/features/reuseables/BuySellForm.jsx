...
import { SubmitButton, NormalDropdown, SearchDropdown, DateInput } from '../../components';
...
const BuySellForm = ({ close, buyForm, data, isEditing, darkMode }) => {
  ...
  return (
    <div className='fixed pt-[23px] px-[19px] dark:bg-[#1E252A] bg-primary h-full top-[50px] left-0 right-0 bottom-0 rounded-t-[32px]'>
      ...
      {buyForm && (
        <NormalDropdown iconName={darkMode ?  'DownBlack.svg': 'DownWhite.svg'} options={options} selectedOption={selectedFormType} onSelect={handleFormTypeSelect} alignment={'origin-top-left left-0'} />
      )}
      ...
      <form className='flex flex-col gap-[24px] mt-[18px]' onSubmit={handleSubmit(onSubmit)}>
        ...
        <label className="relative flex flex-col gap-[8px]">
          ...
          <DateInput selected={selectedDate} onChange={handleDateChange} />
        </label>
